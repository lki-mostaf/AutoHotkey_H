...
EXPORT(int)
ahkReady(DWORD aThreadID) // HotKeyIt check if dll is ready to execute
{
...
EXPORT(int)
ahkPause(int aNewState, DWORD aThreadID) // Change pause state of a running script
{
#pragma comment(linker, "/export:" __FUNCTION__ "=" __FUNCDNAME__)
	AutoTLS atls;
...
EXPORT(UINT_PTR)
ahkFindFunc(LPTSTR aFuncName, DWORD aThreadID)
{
#pragma comment(linker, "/export:" __FUNCTION__ "=" __FUNCDNAME__)
	AutoTLS atls;
...
EXPORT(UINT_PTR)
ahkFindLabel(LPTSTR aLabelName, DWORD aThreadID)
{
#pragma comment(linker, "/export:" __FUNCTION__ "=" __FUNCDNAME__)
	AutoTLS atls;
	if (atls.Enter(aThreadID))
...
// Naveen: v1. ahkGetVar()
EXPORT(LPTSTR)
ahkGetVar(LPTSTR name, int getVar, DWORD aThreadID)
{
#pragma comment(linker, "/export:" __FUNCTION__ "=" __FUNCDNAME__)
	AutoTLS atls;
	if (atls.Enter(aThreadID))
...
EXPORT(int)
ahkAssign(LPTSTR name, LPTSTR value, DWORD aThreadID)
{
#pragma comment(linker, "/export:" __FUNCTION__ "=" __FUNCDNAME__)
	AutoTLS atls;
	if (atls.Enter(aThreadID))
...
EXPORT(UINT_PTR)
ahkExecuteLine(UINT_PTR line, int aMode, int wait, DWORD aThreadID)
{
#pragma comment(linker, "/export:" __FUNCTION__ "=" __FUNCDNAME__)
	if (line == NULL)
		return 0;
	...
	if (atls.Enter(aThreadID))
	{
		...
		if (aMode)
		{
			if (!hwnd || wait && hwnd == g_hWnd)
				templine->ExecUntil(aMode == ONLY_ONE_LINE ? ONLY_ONE_LINE : aMode == UNTIL_BLOCK_END ? UNTIL_BLOCK_END
																									  : UNTIL_RETURN);
			else if (wait)
...
EXPORT(int)
ahkLabel(LPTSTR aLabelName, int nowait, DWORD aThreadID) // 0 = wait = default
{
#pragma comment(linker, "/export:" __FUNCTION__ "=" __FUNCDNAME__)
	AutoTLS atls;
	if (atls.Enter(aThreadID))
	...
	void Restore()
	{
		...
		*mCurrentModuleImportEndPtr = nullptr;
		...
		if (g_LastHotCriterion = mLastHotCriterion)
			...
			hc = g_FirstHotCriterion, g_FirstHotCriterion = nullptr;
		if (g_LastHotExpr = mLastHotExpr)
			mLastHotExpr->NextExpr = nullptr;
		else
			g_FirstHotExpr = nullptr;
...
EXPORT(UINT_PTR)
addScript(LPTSTR script, int waitexecute, DWORD aThreadID)
{
#pragma comment(linker, "/export:" __FUNCTION__ "=" __FUNCDNAME__)
...
EXPORT(int)
ahkExec(LPTSTR script, DWORD aThreadID)
{
#pragma comment(linker, "/export:" __FUNCTION__ "=" __FUNCDNAME__)
...
LPTSTR ahkFunction(LPTSTR func, LPTSTR param1, LPTSTR param2, LPTSTR param3, LPTSTR param4, LPTSTR param5, LPTSTR param6, LPTSTR param7, LPTSTR param8, LPTSTR param9, LPTSTR param10, DWORD aThreadID, int sendOrPost)
{
	...
	for (int i = 0; i < aParamsCount; i++)
	{
		...
		if (*params[i])
			...
			paramtoken[i].symbol = SYM_MISSING;
	}
	atls.~AutoTLS();
	if (sendOrPost)
	{
...
EXPORT(LPTSTR)
ahkFunction(LPTSTR func, LPTSTR param1, LPTSTR param2, LPTSTR param3, LPTSTR param4, LPTSTR param5, LPTSTR param6, LPTSTR param7, LPTSTR param8, LPTSTR param9, LPTSTR param10, DWORD aThreadID)
{
#pragma comment(linker, "/export:" __FUNCTION__ "=" __FUNCDNAME__)
...
EXPORT(int)
ahkPostFunction(LPTSTR func, LPTSTR param1, LPTSTR param2, LPTSTR param3, LPTSTR param4, LPTSTR param5, LPTSTR param6, LPTSTR param7, LPTSTR param8, LPTSTR param9, LPTSTR param10, DWORD aThreadID)
{
#pragma comment(linker, "/export:" __FUNCTION__ "=" __FUNCDNAME__)
...
EXPORT(DWORD)
NewThread(LPTSTR aScript, LPTSTR aCmdLine, LPTSTR aTitle) // HotKeyIt check if dll is ready to execute
{
#pragma comment(linker, "/export:" __FUNCTION__ "=" __FUNCDNAME__)
...
EXPORT(void)
ahkSuppressUserKeys(bool aSuppress)
{
#pragma comment(linker, "/export:" __FUNCTION__ "=" __FUNCDNAME__)
	SuppressUserKeys(aSuppress);
}
...
STDMETHODIMP IAhkApi::QueryInterface(REFIID riid, void **ppv)
{
